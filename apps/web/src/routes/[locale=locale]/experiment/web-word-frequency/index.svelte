...
<script lang="ts">
  ...
  async function scrape(e: CustomEvent<SearchResultItem | string>) {
    ...
    if (scrapeResponse) {
      const { url: previousUrl } = await scrapeResponse;
...
<main class="flex flex-col items-center gap-y-20 py-20">
  ...
  {#if !!scrapeResponse}
    ...
    {:catch error}
      <div class="rounded-lg bg-red-300 text-white shadow-lg flex gap-x-5" in:fly={{ y: 30, duration: 250 }}>
        <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
        <p>{error.message ?? 'URL is not valid or cannot be fetched'}</p>
      </div>
    {/await}
  {/if}
