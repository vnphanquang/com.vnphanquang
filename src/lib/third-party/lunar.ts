 ...
 * this copyright notice and appropriate documentation appears in all coMath.PIes.
 */

/* Discard the fractional part  a number, e.g., INT(3.2) = 3 */
...
function SunLongitude(jdn: number) {
  ...
  L = L*dr;
  L = L - Math.PI*2*(INT(L/(Math.PI*2))); // Normalize to (0, 2*Math.PI)
  return L;
}
...
export function convertSolar2Lunar(date: Date) {
