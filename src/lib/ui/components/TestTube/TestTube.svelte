<script lang="ts">
  export let variant: 'static' | 'progress' = 'static';
  export let percentage = variant === 'static' ? 50 : 20;
  export let color = '#8ec07c';
  export let label = variant === 'progress';
</script>

<div
  class="tube {$$props.class}"
  style="
    --tube-title: '{percentage}%';
    --tube-percentage: {percentage}%;
    --tube-liquid-color: {color};
    --tube-color: currentColor;
  "
>
  <div class="shine" />
  <div class="body">
...
<style>
  ...
  .tube .liquid .percentage::before {
    ...
    left: 50%;
    top: 0;
    background: var(--tube-liquid-color);
    transition: 1s;
  }
  ...
  .tube .meter.label::after,
  .tube .meter.label::before {
  ...
  .tube .meter.label::after {
    ...
    border-right-color: var(--tube-liquid-color);
    padding: 0;
    border-radius: 0;
